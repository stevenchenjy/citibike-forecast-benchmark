...
The global Poisson GLM uses a fixed `PoissonRegressor(alpha=0.1)` after feature standardization. This fixed regularization avoids multiplying the large CPU-only global fit by a GLM parameter sweep; validation-based parameter search is retained for LightGBM as required.
