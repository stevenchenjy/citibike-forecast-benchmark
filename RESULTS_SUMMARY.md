...
- The gated compact Poisson GRU used only no-weather demand histories. It reaches 3.4463 two-hour MAE (better than historical average but not LightGBM) and 4.4192 day-ahead MAE (worse than historical average), with 71,172 primary-ordering inventory failures. Its result is retained without post-hoc tuning; the Mac PyTorch runtime required the documented CPU batch-size-16 fallback after native MPS/large-batch failures.
