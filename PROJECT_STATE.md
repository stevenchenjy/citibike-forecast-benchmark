...
- The publication branch versions the accepted core, weather, and GRU figures plus the linked machine-readable result tables, so GitHub renders the research results without requiring a local reproduction run. Smoke-only integration assets remain generated-only.

Last verified action: the GitHub publication package was assembled and link-checked across `README.md`, `docs/RESEARCH_REPORT.md`, and `reports/final_report.md`. The repository versions the reader-facing figures and linked result tables; generated raw inputs, predictions, checkpoints, and manifests remain reproducible local artifacts. The test suite passed 21 tests.
